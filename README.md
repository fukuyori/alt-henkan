...
Emacsモードでも文字・数字・記号の入力は通常どおりです。次の移動ショートカットのみ変換し、それぞれを個別にON／OFFできます（各操作の既定値はON）。
...
実際の移動単位は対象アプリの矢印・Home・End・Ctrl+矢印の動作に従います。ONの操作は対象アプリ本来のショートカットより優先します。たとえばCtrl+Aの全選択、Ctrl+Pの印刷、Ctrl+Fの検索は該当項目をOFFにすると使えます。Shift・Windowsキーなどを追加した組み合わせ、Ctrl+Vなど上記以外のショートカットは変換しません。viモードや文字編集の操作は含みません。

Alt+B／Alt+Fを移動として使った後にAltを離しても、IME切り替えを送りません。それ以外のAlt単独操作・組み合わせは従来の動作を保ちます。
...
左右の Alt を一度ずつ押し、通知領域のメニューからアプリを終了して、`%LOCALAPPDATA%\AltHenkan\AltHenkan-diagnostics.log` を確認してください。ログにはAltの処理、モード切り替え、Emacs移動操作の種類、入力注入の結果が記録され、通常入力の文字は記録されません。
